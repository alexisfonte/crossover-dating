class Message < ApplicationRecord  
    ...
    def self.new_message(params)
      viewed_user = User.find(params[:recipient_id])
      uuid = Match.where(user_id: params[:sender_id]).where(viewed_user_id: params[:recipient_id])[0][:pair_id]
    ...
    def self.messages(params)
      pair_id = Match.all.where(user_id: params[:sender_id]).where(viewed_user_id: params[:recipient_id])[0][:pair_id]
    ...
    def self.list_messages(user, matched_users)
      pair_ids = matched_users.map do |matched_user|
        Match.all.where(user_id: user[:id]).where(viewed_user_id: matched_user[:id])[0][:pair_id]
      end
